...
def create_app(database_url: Optional[str] = None, interactive_model_selection: bool = True):
    ...
    def _generate_intel_kg_impl():
        ...
        try:
            ...
            # Try to get relevant items first, but fallback to all items
            query = session.query(IntelItem).filter(
                IntelItem.is_relevant == True
            )
            ...
            print(f"[API] Found {len(relevant_items)} relevant items")
            
            if len(relevant_items) > 0:
                items = relevant_items
            else:
                # Use all items if no relevant ones
                print(f"[API] No relevant items found, using all items")
                query = session.query(IntelItem).order_by(
                    IntelItem.created_at.desc()
                )
                if limit and limit > 0:
                    query = query.limit(limit)
                items = query.all()
                print(f"[API] Using {len(items)} total items")
            ...
            try:
                ...
                if temp_kg_manager.data_dir.exists():
                    ...
                    if files:
                        ...
                        # consume generator to load
                        for _ in builder.load_from_mcp_graph(existing_graph):
                            pass
