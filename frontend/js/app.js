...
function showNodeDetails(nodeData) {
    ...
    if (sourceUrls.length > 0) {
        const urlListItems = sourceUrls.map((url, idx) => {
            const isPrimary = idx === 0 && url === primaryUrl;
            let domain = url;
            try { domain = new URL(url).hostname.replace(/^www\./, ''); } catch(e) {}
            const truncatedUrl = url.length > 55 ? url.substring(0, 55) + '…' : url;
            return `
                <div style="display: flex; align-items: center; gap: 8px; padding: 8px 10px; border-radius: 6px; border: 1px solid var(--border-color); background: var(--bg-primary); margin-bottom: 6px;">
                    <div style="flex: 1; min-width: 0;">
                        ${isPrimary ? '<div style="font-size: 0.7rem; color: #f59e0b; font-weight: 600; margin-bottom: 2px;">⭐ Primary</div>' : ''}
                        <div style="font-size: 0.8rem; font-weight: 600; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${domain}</div>
                        <div style="font-size: 0.72rem; color: var(--text-secondary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${url}">${truncatedUrl}</div>
                    </div>
                    <a href="${url}" target="_blank" rel="noopener noreferrer"
                       style="flex-shrink: 0; display: flex; align-items: center; justify-content: center; width: 30px; height: 30px; border-radius: 6px; background: #3b82f6; color: white; text-decoration: none; font-size: 0.875rem; transition: background 0.15s;"
        ...
        html += `
            <div style="margin-bottom: 16px;">
                <div style="font-size: 0.875rem; color: var(--text-secondary); margin-bottom: 8px;">
                    Source URLs <span style="font-weight: normal;">(${sourceUrls.length} total)</span>
                </div>
