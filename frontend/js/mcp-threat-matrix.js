...
/**
 * Show threat details in a rich modal
 */
function showThreatDetails(threatId) {
    ...
    // Extract nested schema_data fields
    const schema = (typeof threat.schema_data === 'string')
        ? (() => { try { return JSON.parse(threat.schema_data); } catch(e) { return {}; } })()
        : (threat.schema_data || {});

    const riskLevel   = threat.risk_level   || schema.risk_level   || 'Unknown';
    const riskScore   = threat.risk_score   || schema.risk_score   || '';
    const stride      = threat.stride_category || schema.stride_category || 'N/A';
    const phase       = threat.mcp_workflow_phase || schema.mcp_workflow_phase || 'N/A';
    const attackType  = threat.msb_attack_type || schema.msb_attack_type || 'N/A';
    const vector      = threat.threat_vector || schema.threat_vector || 'N/A';

    const attackSteps   = threat.attack_steps   || schema.attack_steps   || [];
    const detections    = threat.detection_methods || schema.detection_methods || [];
    const mitigations   = threat.mitigations    || schema.mitigations    || [];
    const assets        = threat.assets_at_risk || schema.assets_at_risk || [];
    const preconditions = threat.preconditions  || schema.preconditions  || [];

    // Source intel IDs → find matching intel items
    const sourceIntelIds = threat.source_intel_ids || schema.source_intel_ids || [];
    const sourceIntelItems = (MCPThreatMatrixState.intelItems || []).filter(i =>
        sourceIntelIds.includes(String(i.id)) || sourceIntelIds.includes(i.id)
    );

    const riskClass = riskLevel.toLowerCase();

    // Remove any existing threat detail modal
    document.querySelectorAll('.threat-detail-modal').forEach(m => m.remove());
    ...
    modal.innerHTML = `
        <div style="
            ...
            border-radius: 12px; max-width: 860px; width: 100%; max-height: 90vh;
            overflow-y: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.5);
        ">
            <!-- Header -->
            <div style="
                display: flex; justify-content: space-between; align-items: flex-start;
                padding: 20px 24px 16px; border-bottom: 1px solid var(--border-color, #2a2a4a);
                position: sticky; top: 0; background: var(--bg-primary, #1a1a2e); z-index: 1;
            ">
                <div>
                    <h3 style="margin: 0 0 6px; font-size: 1.15rem; color: var(--text-primary, #e2e8f0);">
                        ${escapeHtml(threat.name || threat.title || 'Unknown Threat')}
                    </h3>
                    <div style="display: flex; gap: 8px; flex-wrap: wrap; align-items: center;">
                        <span class="risk-badge risk-${riskClass}" style="
                            padding: 3px 10px; border-radius: 20px; font-size: 0.78rem; font-weight: 600;
                            background: ${riskClass === 'critical' ? 'rgba(220,53,69,0.25)' : riskClass === 'high' ? 'rgba(255,140,0,0.25)' : riskClass === 'medium' ? 'rgba(255,193,7,0.2)' : 'rgba(40,167,69,0.2)'};
                            color: ${riskClass === 'critical' ? '#ff6b7a' : riskClass === 'high' ? '#ffaa40' : riskClass === 'medium' ? '#ffd700' : '#5cb85c'};
                        ">${riskLevel.toUpperCase()}${riskScore ? ' · ' + riskScore : ''}</span>
                        <span style="font-size: 0.8rem; color: var(--text-secondary, #8892a4);">STRIDE: ${escapeHtml(stride)}</span>
                        <span style="font-size: 0.8rem; color: var(--text-secondary, #8892a4);">Phase: ${escapeHtml(phase)}</span>
                    </div>
                </div>
                <button onclick="this.closest('.threat-detail-modal').remove()" style="
                    background: none; border: none; color: var(--text-secondary, #8892a4);
                    font-size: 1.4rem; cursor: pointer; line-height: 1; padding: 0; margin-left: 16px;
                ">×</button>
            </div>
            ...
            <div style="padding: 20px 24px; display: flex; flex-direction: column; gap: 20px;">
                ...
                <!-- Description -->
                <div>
                    <div style="font-size: 0.75rem; font-weight: 600; color: var(--accent-primary, #6c63ff); text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 6px;">Description</div>
                    <p style="margin: 0; color: var(--text-primary, #e2e8f0); font-size: 0.9rem; line-height: 1.6;">
                        ${escapeHtml(threat.description || 'No description available.')}
                    </p>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 12px;">
                    ${[
                        ...
                        ['MCP Workflow Phase', phase],
                    ].map(([label, val]) => `
                        <div style="background: var(--bg-secondary, #16213e); border-radius: 8px; padding: 10px 12px;">
                            <div style="font-size: 0.72rem; color: var(--text-muted, #5a637a); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px;">${escapeHtml(label)}</div>
                            <div style="font-size: 0.85rem; color: var(--text-primary, #e2e8f0);">${escapeHtml(val || 'N/A')}</div>
                        </div>
                    `).join('')}
                </div>

                <!-- Assets & Preconditions -->
                ${assets.length || preconditions.length ? `
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                    ${assets.length ? `
                    <div>
                        <div style="font-size: 0.75rem; font-weight: 600; color: var(--accent-primary, #6c63ff); text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 8px;">Assets at Risk</div>
                        <ul style="margin: 0; padding-left: 16px; color: var(--text-primary, #e2e8f0); font-size: 0.85rem;">
                            ${assets.map(a => `<li style="margin-bottom: 4px;">${escapeHtml(String(a))}</li>`).join('')}
                        </ul>
                    </div>` : ''}
                    ${preconditions.length ? `
                    <div>
                        <div style="font-size: 0.75rem; font-weight: 600; color: var(--accent-primary, #6c63ff); text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 8px;">Preconditions</div>
                        <ul style="margin: 0; padding-left: 16px; color: var(--text-primary, #e2e8f0); font-size: 0.85rem;">
                            ${preconditions.map(p => `<li style="margin-bottom: 4px;">${escapeHtml(String(p))}</li>`).join('')}
                        </ul>
                    </div>` : ''}
                </div>` : ''}
                ...
                ${attackSteps.length ? `
                <div>
                    <div style="font-size: 0.75rem; font-weight: 600; color: #ed8936; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 8px;">⚔️ Attack Steps</div>
                    <div style="display: flex; flex-direction: column; gap: 8px;">
                        ${attackSteps.map((s, i) => `
                        <div style="display: flex; gap: 10px; background: var(--bg-secondary, #16213e); border-radius: 8px; padding: 10px 12px;">
                            <div style="
                                min-width: 24px; height: 24px; border-radius: 50%;
                                background: rgba(237,137,54,0.2); color: #ed8936;
                                display: flex; align-items: center; justify-content: center;
                                font-size: 0.75rem; font-weight: 700; flex-shrink: 0;
                            ">${(s.step_number || i + 1)}</div>
                            <div>
                                <div style="font-size: 0.85rem; color: var(--text-primary, #e2e8f0);">${escapeHtml(s.action || String(s))}</div>
                                ${s.expected_result ? `<div style="font-size: 0.78rem; color: var(--text-secondary, #8892a4); margin-top: 3px;">→ ${escapeHtml(s.expected_result)}</div>` : ''}
                            </div>
                        </div>`).join('')}
                    </div>
                </div>` : ''}
                ...
                ${detections.length ? `
                <div>
                    <div style="font-size: 0.75rem; font-weight: 600; color: #4299e1; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 8px;">🔍 Detection Methods</div>
                    <div style="display: flex; flex-direction: column; gap: 6px;">
                        ${detections.map(d => `
                        <div style="background: var(--bg-secondary, #16213e); border-radius: 8px; padding: 10px 12px; font-size: 0.85rem; color: var(--text-primary, #e2e8f0);">
                            ${typeof d === 'object' ? `<strong>${escapeHtml(d.method_type || 'Detection')}:</strong> ${escapeHtml(d.description || '')}` : escapeHtml(String(d))}
                        </div>`).join('')}
                    </div>
                </div>` : ''}
                ...
                ${mitigations.length ? `
                <div>
                    <div style="font-size: 0.75rem; font-weight: 600; color: #48bb78; text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 8px;">🛡️ Mitigations</div>
                    <ul style="margin: 0; padding-left: 16px; display: flex; flex-direction: column; gap: 5px;">
                        ${mitigations.map(m => `
                        <li style="font-size: 0.85rem; color: var(--text-primary, #e2e8f0);">
                            ${typeof m === 'object' ? `<strong>${escapeHtml(m.control_type || '')}:</strong> ${escapeHtml(m.description || '')}` : escapeHtml(String(m))}
                        </li>`).join('')}
                    </ul>
                </div>` : ''}

                <!-- Source Intel -->
                ${sourceIntelItems.length ? `
                <div>
                    <div style="font-size: 0.75rem; font-weight: 600; color: var(--text-secondary, #8892a4); text-transform: uppercase; letter-spacing: 0.06em; margin-bottom: 8px;">📡 Source Intelligence</div>
                    <div style="display: flex; flex-direction: column; gap: 6px;">
                        ${sourceIntelItems.map(i => {
                            const url = i.source_url || i.url || '';
                            const validUrl = url && url.startsWith('http');
                            return `
                            <div style="background: var(--bg-secondary, #16213e); border-radius: 8px; padding: 10px 12px; font-size: 0.85rem;">
                                ${validUrl
                                    ? `<a href="${escapeHtml(url)}" target="_blank" rel="noopener" style="color: var(--accent-primary, #6c63ff); text-decoration: none;">${escapeHtml(i.title || url)}</a>`
                                    : `<span style="color: var(--text-primary, #e2e8f0);">${escapeHtml(i.title || 'Untitled')}</span>`
                                }
                                ${i.ai_summary ? `<div style="font-size: 0.78rem; color: var(--text-secondary, #8892a4); margin-top: 4px;">${escapeHtml((i.ai_summary || '').substring(0, 150))}${i.ai_summary.length > 150 ? '...' : ''}</div>` : ''}
                            </div>`;
                        }).join('')}
                    </div>
                </div>` : ''}
...
async function showOWASPDetails(type, owaspId) {
    try {
        ...
        modal.innerHTML = `
            ...
            const summary = (i.ai_summary || i.content || '').trim();
            return `
                    ...
                    escapeHtml(i.title || 'Untitled')
                                    ...
                                    ${summary ? `<div style="font-size: 0.85rem; color: var(--text-secondary); margin-top: 4px; line-height: 1.5;">${escapeHtml(summary)}</div>` : ''}
                                    <div style="margin-top: 6px;">
                                        ...
                                        ${isValidUrl ? `<a href="${sourceUrl}" target="_blank" rel="noopener noreferrer" style="margin-left: 8px; font-size: 0.75rem; color: var(--accent-primary);">🔗 Source</a>` : ''}
                                    </div>
                                </div>
                            `;
        }).join('') :
