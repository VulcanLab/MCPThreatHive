...
_FUZZY_THRESHOLD = 0.85  # Jaccard threshold for auto-merge
...
class IntelKnowledgeGraphBuilder:
    ...
    def load_from_mcp_graph(self, graph: MCPKnowledgeGraph):
        ...
        for edge in graph.edges:
            ...
            # A better way would be using a relation ID or set, but relations list is okay for now
        ...
        # Pre-filter items with valid text
        valid_items = []
        for item in intel_items:
            summary = item.get('ai_summary') or item.get('content', '')
            ...
            valid_items.append((item, item_id, summary[:2000]))
        ...
        if use_ai:
            ...
            for batch_start in range(0, len(valid_items), BATCH_SIZE):
                ...
                for idx, (item, item_id, summary) in enumerate(batch):
                    ...
                    if batch_results and idx in batch_results:
                        ...
                        # Fallback to simple extraction for this item
                        entities, relations = self._extract_simple(summary, item_id)
            ...
            # Non-AI path: simple extraction per item
            for item, item_id, summary in valid_items:
                processed_count += 1
                entities, relations = self._extract_simple(summary, item_id)
                for entity in entities:
                    self._add_entity(entity)
    ...
    def _extract_batch_with_ai(self, batch: List[Tuple]) -> Optional[Dict[int, Tuple[List[Entity], List[Relation]]]]:
        ...
        existing_names = list(self.entity_name_to_id.keys())[:30]  # Top 30 known entities
...
IMPORTANT: Reuse these EXISTING entity names if they match: [{existing_names_str}]
Do NOT create new entity names that are synonyms or variations of existing ones.
...
{{
    ...
    def _extract_simple(self, text: str, source_id: str) -> Tuple[List[Entity], List[Relation]]:
        ...
        # MCP-specific patterns for pattern-based extraction
        mcp_patterns = {
            "Threat": [
        ...
        # Extract capitalized terms (MCP, API, etc.)
        capitalized_terms = re.findall(r'\b(?:MCP|API|LLM|AI|CVE|CWE|STRIDE|MAESTRO)\b', text, re.IGNORECASE)
        for term in set(capitalized_terms):
            entity_id = self._generate_entity_id(term.upper())
            if entity_id not in extracted_patterns:
                entity = Entity(
                    id=entity_id,
                    name=term.upper(),
                    entity_type="Component",
                    description=f"{term.upper()} component or framework",
                    source_ids=[source_id]
                )
                entities.append(entity)
                extracted_patterns[entity_id] = entity
        ...
        # Create hub connections - connect all entities that appear in same text to a central "MCP Security" hub
        if len(all_entities) > 2:
            # Find or create a central hub entity
            hub_id = self._generate_entity_id("MCP Security")
            hub_exists = hub_id in [e.id for e in all_entities]
            ...
            if not hub_exists:
                hub_entity = Entity(
                    ...
                    name="MCP Security",
                    entity_type="Component",
                    description="MCP Security Knowledge Hub",
                    source_ids=[source_id]
                )
                entities.append(hub_entity)
                all_entities.append(hub_entity)
            
            # Connect significant entities to hub
            for entity in all_entities[:10]:  # Connect top 10 entities to hub
                if entity.id != hub_id:
                    relation = Relation(
                        source_id=entity.id,
                        target_id=hub_id,
                        relation_type="RELATED_TO",
                        description=f"{entity.name} related to MCP Security",
                        source_ids=[source_id]
                    )
                    relations.append(relation)
    ...
    def _add_entity(self, entity: Entity):
        ...
        # Stage 2: Fuzzy Jaccard similarity match against all existing entities
    ...
    def _add_relation(self, relation: Relation):
        """Add relation (check if entities exist)"""
        # Ensure source and target entities exist
        if relation.source_id not in self.entities:
            # Create placeholder entity
            self.entities[relation.source_id] = Entity(
                id=relation.source_id,
                name=relation.source_id.replace("entity-", "").replace("-", " ").title(),
                entity_type="Entity",
                description=""
            )
        ...
        if relation.target_id not in self.entities:
            # Create placeholder entity
            self.entities[relation.target_id] = Entity(
                id=relation.target_id,
                name=relation.target_id.replace("entity-", "").replace("-", " ").title(),
                entity_type="Entity",
                description=""
            )
    ...
    def _build_mcp_graph(self) -> MCPKnowledgeGraph:
        ...
        if len(graph.nodes) > 0:
            ...
            # If hub is new/isolated, it's its own component. 
            ...
            for i, comp in enumerate(components):
                ...
                # Find the best representative node in this isolated component to link
                # Priority: "Threat" > "Technique" > "Component" > Any
                # Or just the one with the most edges within its own component (highest degree)
                best_node_id = comp[0]
                best_degree = -1
